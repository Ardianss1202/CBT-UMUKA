        ...
        <div class="card shadow-lg">
            <div class="p-2 text-white d-flex justify-content-between align-items-center" style="background-color:#254baab3">
                ...
                {{-- <div class="d-flex gap-2 ms-auto"> 
                    <a href="{{route('ujian.create')}}" class="btn btn-sm" style="background-color:#0ee612e1; color:white">Tambah</a>
                </div> --}}
            </div>
                    ...
                    <table class="table table-bordered table-hover table-striped">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>{{ $ujian->token }}</td>
                                <td>
                                    <form action="{{ route('ujian.generateToken', $ujian->id) }}" method="POST" style="display:inline;">
                                        @csrf
                                        @method('PUT')
                                        <button type="submit" class="btn btn-sm btn-success">Refresh Token</button>
                                    </form>
                                    <a href="{{ url('/edit-data-ujian/' . $ujian->id) }}" class="btn btn-primary btn-sm">Edit</a>
                                    {{-- <a href="{{ url('/ujian/' . $ujian->id . '/edit') }}" class="btn btn-primary btn-sm">Edit</a>
                        
                                    <form action="{{ route('ujian.destroy', $ujian->id) }}" method="POST" style="display:inline;" onsubmit="return confirm('Apakah Anda yakin ingin menghapus data ini?');">
                                        @csrf
                                        @method('DELETE')
